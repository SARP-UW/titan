...
int safe_state_run(){
    ...
    if (baro1_result.pressure < SAFE_PRESSURE_THRESHOLD && baro2_result.pressure < SAFE_PRESSURE_THRESHOLD) {
    ...
    if (errc && errc != TI_ERRC_NONE) {
        TI_SET_ERRC(&errc, errc, "Failed to build safe sensor packet");
    }
    ...
    if (errc && errc != TI_ERRC_NONE) {
        TI_SET_ERRC(&errc, errc, "Failed to send safe sensor packet");
    }
    ...
    build_comm_packet(state_comm_shared.ping_id, SAFE_STATE_IDX,
    ...
    comm_result_t cmd_result = process_command(&state_comm_shared.uplink_comm, SAFE_STATE_IDX,
    ...
    if (cmd_result.valid) {
        switch (cmd_result.action) {
            ...
            case COMM_ACTION_SET_MODE:
                ...
                if (cmd_result.param1 == 1) { // STANDBY_STATE_IDX
