...
int hold_state_run(){
    ...
    if (errc && errc != TI_ERRC_NONE) {
        TI_SET_ERRC(&errc, errc, "Failed to build hold sensor packet");
    }
    ...
    if (errc && errc != TI_ERRC_NONE) {
        TI_SET_ERRC(&errc, errc, "Failed to send hold sensor packet");
    }

    // Send ADC data packet (0x03)
    build_adc_packet(adc_channels, adc_channel_count, ADC_PACKET_INDEX,
                     state_comm_shared.adc_packet, sizeof(state_comm_shared.adc_packet), &errc);
    if (errc && errc != TI_ERRC_NONE) {
    ...
    build_comm_packet(state_comm_shared.ping_id, HOLD_STATE_IDX,
    ...
    comm_result_t cmd_result = process_command(&state_comm_shared.uplink_comm, HOLD_STATE_IDX,
    ...
    if (cmd_result.valid) {
        switch (cmd_result.action) {
            ...
            case COMM_ACTION_SET_MODE:
                if (cmd_result.param1 == HOLD_STATE_IDX || cmd_result.param1 == ARMED_STATE_IDX || cmd_result.param1 == MAX_STATE_INDEX) {
