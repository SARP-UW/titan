...
static const int kRs485DePin = 43;
static const int kRs485RePin = 44;
...
void _start() { // NOLINT(misc-use-internal-linkage)
    ...
    tal_set_mode(kRs485DePin, 1);
    tal_set_mode(kRs485RePin, 1);
    tal_set_pin(kRs485DePin, 1);
    tal_set_pin(kRs485RePin, 0);
    ...
    while (1) {
        size_t rx_len = 0U;
        memset(state_comm_shared.rx_packet, 0, sizeof(state_comm_shared.rx_packet));
