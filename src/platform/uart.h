...
/**
 * @brief Sends data over the specified UART channel. Blocking (syncronous) function.
 *
 * @param channel USART channel
 * @param tx_buff Pointer to the data buffer to be transmitted.
 * @param size Number of bytes to transmit.
 ...
 * @return true if data transmission is successful, false otherwise.
 */
bool uart_write_blocking(tal_flag_t *flag, uart_channel_t channel, uint8_t* tx_buff, uint32_t size);

/**
 * @brief Receives data from the specified UART channel. Blocking (syncronous) function.
 *
 * @param channel USART channel
 * @param rx_buff Pointer to the buffer where received data will be stored.
 * @param size Number of bytes to read.
