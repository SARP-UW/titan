...
{ 
    ...
    WRITE_FIELD(I2Cx_CR1[0], I2Cx_CR1_PE, 0);
    
    // for now disable digital and analog noise filters, we can chose to have these later
    WRITE_FIELD(I2Cx_CR1[0], I2Cx_CR1_DNF, 0);
    WRITE_FIELD(I2Cx_CR1[0], I2Cx_CR1_ANFOFF, 0);

    uint32_t timing = 0x00707CBB; // generated by STMCube, subject to change
    *I2Cx_TIMINGR[0] = timing;

    WRITE_FIELD(I2Cx_CR1[0], I2Cx_CR1_PE, 1);
}
...
static void tal_transmit_r(uint8_t addr, void* d, uint32_t size, bool first_call)
{
    ...
    if(size <= 255){
        WRITE_FIELD(I2Cx_CR2[0], I2Cx_CR2_NBYTES, size);
        data_size = size;
    }else{
        WRITE_FIELD(I2Cx_CR2[0], I2Cx_CR2_NBYTES, 255);
        WRITE_FIELD(I2Cx_CR2[0], I2Cx_CR2_RELOAD, 1);
        data_size = 255;
    }

    if(first_call){
        // set addr mode, default 7 bit, so do nothing
        WRITE_FIELD(I2Cx_CR2[0], I2Cx_CR2_SADD_7BIT, addr);
        WRITE_FIELD(I2Cx_CR2[0], I2Cx_CR2_RD_WRN, 0);
        WRITE_FIELD(I2Cx_CR2[0], I2Cx_CR2_AUTOEND, 1);
        WRITE_FIELD(I2Cx_CR2[0], I2Cx_CR2_START, 1);
    }
    ...
    while(data_size > 0){
        ...
        while(READ_FIELD(I2Cx_ISR[0], I2Cx_ISR_TXIS) != 1 && 
        count++ < timeout); // wait until TXIS is set
        
        WRITE_FIELD(I2Cx_TXDR[0], I2Cx_TXDR_TXDATA, data);
...
static void* tal_read_r(uint8_t addr, uint32_t size, bool first_call, uint8_t* data, uint32_t i)
{
    ...
    if(first_call){
        ...
        WRITE_FIELD(I2Cx_CR2[0], I2Cx_CR2_RD_WRN, 1); // 1 for read 
        WRITE_FIELD(I2Cx_CR2[0], I2Cx_CR2_AUTOEND, 1);
        WRITE_FIELD(I2Cx_CR2[0], I2Cx_CR2_START, 1);
    }
    ...
    while(data_size > 0){
        uint32_t count = 0;
        while(READ_FIELD(I2Cx_ISR[0], I2Cx_ISR_RXNE) != 1 && 
        count++ < timeout); // wait until RXNE is set
        
        uint8_t d = READ_FIELD(I2Cx_RXDR[0], I2Cx_RXDR_RXDATA);
        data[i] = d;
