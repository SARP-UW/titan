...
int dma_configure_stream(const dma_stream_config_t* config) {
    ...
    WRITE_FIELD(dma_cr_map[config->instance][config->stream], DMAx_SxCR_MINC, config->periph_inc);

    // Disable peripheral increment (we don't want to increment peripheral registers)
    WRITE_FIELD(dma_cr_map[config->instance][config->stream], DMAx_SxCR_PINC, config->mem_inc);
